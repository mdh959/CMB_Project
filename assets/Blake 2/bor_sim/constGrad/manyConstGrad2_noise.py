...
# data saved in:
outf = 'out_n0_1_noise_'+str(myrank)+'.txt'
...
phi.data[:,:] = phi.data[:,:] - mean  # CRUCIAL

# setting temperature map in uK
Tulens = phi.copy()
Tulens.data[:,:] = np.array([np.linspace(-1350,1350,2048),]*2048) # necessarily in uK units because grad is 13 uK/arcmin

# lens T
Tlens, Qlens, Ulens  =  lensMaps(phi,Tulens, Tulens, Tulens)

# add noise in uK
NlT = (deltaT*thetaFWHM)**2*np.exp(lphi*(lphi+1.)*thetaFWHM**2/(8.*np.log(2.)))*noiseFactor**2.#/tcmb**2
noise = Tlens.copy()
...
# map with est - true
diff.data[:,:] = phi_e_win.data - phi_win.data
...
ps2d_c = fftTools.powerFromLiteMap(diff)
l, l, ll, Nlphi, l, l = ps2d_c.binInAnnuli(bin)
...
for i in range(len(ll)):
    print >> f1, ('% 8.1f' % ll[i]), ('% 18.8e' % Clphi_win[i]), ('% 18.8e' % Clphi_e_win[i]), ('% 18.8e' % Clphi_c[i]), ('% 18.8e' % Nlphi[i])
