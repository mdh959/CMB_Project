...
# WINDOW DEFINITELY WANT WINDOW!!!!
win = liteMapPol.initializeCosineWindow(phi,200,0)#200,0)#better with 200,0)
#win = phi.copy()
#win.data[:,:] = 1.

phi_win = phi.copy()
phi_win.data[:,:] *= win.data
...
dx_e = phi.copy()
# WORKS!!!
#dx_e.data[:,:] = (Tlens.data - Tulens.data)/rms
# WORKS TOO THOUGH SEEMS A TINY BIT WORSE
dx_e.data[:,:] = (Tlens_win.data - Tulens_win.data)/rms

dx_e.plot(show=False,saveFig = '../lala7.png');plt.close()
dx.plot(show=False,saveFig = '../lala8.png');plt.close()

# alternative gradient
# MUCH MUCH BETTER AND WEIRDLY SO
Fdx_f = Fphi.copy()
Fdx_f.kMap[:,:] *= Fphi.lx*1j
# THAN
Fdx_win_f = Fphi_win.copy()
Fdx_win_f.kMap[:,:] *= Fphi_win.lx*1j

dx_win_f = dx.copy()
# SETMEAN FALSE IS BETTER
dx_win_f.data[:,:] = Fdx_win_f.mapFromFFT(setMeanToZero=False)*win.data
dx_win_f.plot(show=False,saveFig = '../lala6.png',valueRange=[-0.00085,0.00085]);plt.close()

dx_f = dx.copy()
# SETMEAN FALSE IS BETTER
dx_f.data[:,:] = Fdx_f.mapFromFFT(setMeanToZero=False)*win.data
dx_f.plot(show=False,saveFig = '../lala5.png',valueRange=[-0.00085,0.00085]);plt.close()

diff_win_win = phi.copy()
# WORKS!!!
#diff_win.data[:,:] = (dx_e.data - dx.data)*rms
# WORKS TOO THOUGH SEEMS A TINY BIT WORSE
#diff_win.data[:,:] = (dx_e.data - dx_win.data)*rms
# works very fine too!!! WITH FOURIER
diff_win_win.data[:,:] = (dx_e.data - dx_win_f.data)*rms
diff_win_win.data[:,:] *= win.data
...
#diff_win.data[:,:] = (dx_e.data - dx_win.data)*rms
# works very fine too!!! WITH FOURIER
diff_win.data[:,:] = (dx_e.data - dx_f.data)*rms
diff_win.data[:,:] *= win.data

diff_win.plot(show=False,saveFig = '../lala9.png');plt.close()
noise_win.plot(show=False,saveFig = '../lala0.png');plt.close()

ps2d = fftTools.powerFromLiteMap(diff_win)
l, l, ll, Clphi_diff, l, l = ps2d.binInAnnuli(bin)

ps2d = fftTools.powerFromLiteMap(diff_win_win)
l, l, ll, Clphi_diff_win, l, l = ps2d.binInAnnuli(bin)
...
Clphi_diff_win *= ll**4/2*np.pi
Clphi_n_win *= ll**4/2*np.pi
NlT *= lphi**4/2*np.pi

#plt.plot(ll,Clphi_n,label='noise')
plt.plot(ll,Clphi_n_win,label='noise win')
plt.plot(ll,Clphi_diff,label='-WinFphi')
plt.plot(ll,Clphi_diff_win,label='+WinFphi')
...
plt.savefig('CHECK100000000.pdf')
plt.close()
quit()
# transforming to d
