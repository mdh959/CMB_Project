...
x0 = 179.75#181.65 #181.25
y0 = 0.125
x1 = 179.9999#178.35 #178.75
y1 = -0.125
...
Fdx = liteMap.fftFromLiteMap(dx)
Fdy = liteMap.fftFromLiteMap(dy)

print np.max(Fphi.lx)
print np.max(Fphi.ly)
print Fphi.Nx
print Fphi.Ny
...
sign_x = np.mean(gradTux.data)/np.abs(np.mean(gradTux.data))
sign_y = np.mean(gradTuy.data)/np.abs(np.mean(gradTuy.data))
...
# WINDOW
win = liteMapPol.initializeCosineWindow(phi,5,0)
#win.data = 1.
...
diff_win.plot(show=False,saveFig = 'diff_T.png');plt.close()
...
lx = np.array([lx,]*Ny)
ly = np.array([ly,]*Nx).transpose()
den = 1j*(lx*rms_x*sign_x+rms_y*ly*sign_y)
den[0,0] = 1.

Fphi_e = Fphi.copy()
Fphi_e.kMap[:,:] = (Fdiff_win.kMap[:,:])/den
Fphi_e.kMap[0,0] = 0. 

# deflection map
Fdx_e = Fphi.copy()
Fdx_e.kMap[:,:] = (Fdiff_win.kMap - sign_y*rms_y*Fdy.kMap)/(rms_x*sign_x)
Fdy_e = Fphi.copy()
Fdy_e.kMap[:,:] = (Fdiff_win.kMap - sign_x*rms_x*Fdx.kMap)/(rms_y*sign_y)

dx_e = phi.copy()
dx_e.data[:,:] = Fdx_e.mapFromFFT(setMeanToZero=True) # TRYING WITHOUT SETTING TO 0

dy_e = phi.copy()
dy_e.data[:,:] = Fdy_e.mapFromFFT(setMeanToZero=True) # TRYING WITHOUT SETTING TO 0
'''
# WORKS PRETTY WELL
# deflection map
dx_e = phi.copy()
dx_e.data[:,:] = (diff_win.data - sign_y*rms_y*dy.data)/(rms_x*sign_x)
dy_e = phi.copy()
dy_e.data[:,:] = (diff_win.data - sign_x*rms_x*dx.data)/(rms_y*sign_y)
'''

phi_e = phi.copy()
phi_e.data[:,:] = Fphi_e.mapFromFFT(setMeanToZero=True) # LET"S NOT SET TO 0
print phi_e.data
phi_e.plot(show=False,saveFig = 'phi_e.png',valueRange=[np.min(phi.data),np.max(phi.data)]);plt.close()
#phi_e.plot(show=False,saveFig = 'phi_e.png');plt.close()
phi.plot(show=False,saveFig = 'phi_r.png');plt.close()

'''
# USE FOR REAL SPACE WHICH WORKS FOURIER GIVES THE SAME
dx_e.plot(show=False,valueRange=[np.min(dx.data),np.max(dx.data)],saveFig = 'dx_e.png');plt.close()
#dx_e.plot(show=False,saveFig = 'dx_e.png');plt.close()
dx.plot(show=False,saveFig = 'dx.png');plt.close()
dy_e.plot(show=False,valueRange = [np.min(dy.data),np.max(dy.data)],saveFig = 'dy_e.png');plt.close()
#dx_e.plot(show=False,saveFig = 'dx_e.png');plt.close()
dy.plot(show=False,saveFig = 'dy.png');plt.close()
'''
dx_e.plot(show=False,valueRange=[np.min(dx.data),np.max(dx.data)],saveFig = 'Fdx_e.png');plt.close()
#dx_e.plot(show=False,saveFig = 'dx_e.png');plt.close()
dx.plot(show=False,saveFig = 'dx.png');plt.close()
dy_e.plot(show=False,valueRange = [np.min(dy.data),np.max(dy.data)],saveFig = 'Fdy_e.png');plt.close()
#dx_e.plot(show=False,saveFig = 'dx_e.png');plt.close()
dy.plot(show=False,saveFig = 'dy.png');plt.close()

# THIS INSTEAD BECAUSE THIS IS WHAT WE REALLY WANT
Fdiff = Fphi.copy()
Fdiff.kMap[:,:] = (Fphi_e.kMap - Fphi.kMap)

Fnoise = Fphi.copy()
# TRY WITH NORMAL NOT WIN BECAUSE SMALL PAK
Fnoise = liteMap.fftFromLiteMap(noise)#noise_win)
Fnoise.kMap[:,:] = Fnoise.kMap/den
Fnoise.kMap[:,0] = 0. # VERY VERY IMPORTANT

diff_win = phi.copy()
diff_win.data[:,:] = Fdiff.mapFromFFT(setMeanToZero=True) # TUKA # TRY MAP TOO SMALL FOR THIS 
