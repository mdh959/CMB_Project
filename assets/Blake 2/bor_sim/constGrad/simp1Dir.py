...
Fphi_e = Fphi.copy()
Fphi_e.kMap[:,:] = (Fdiff_win.kMap[:,:])/(1j*lx*rms)
Fphi_e.kMap[:,0] = 0. # because lx[0] is set to 1

phi_e = phi.copy()
phi_e.data[:,:] = Fphi_e.mapFromFFT(setMeanToZero=True) # TUKA
phi_e.plot(show=False,saveFig = '../lala7.png',valueRange=[-0.000007,0.000007]);plt.close()
phi.plot(show=False,saveFig = '../lala8.png');plt.close()
