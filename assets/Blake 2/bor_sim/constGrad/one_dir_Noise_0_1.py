...
for i in range(N1,N2):
	namef = 'out_n0_1_1d_'+str(i)+'.txt'
	lls, Clphi, Clphi_est_N, Clphi_cross, yerr, Clphi_est_N1d, Clphi1d = np.loadtxt(namef,unpack=True)
	CEST = np.abs((Clphi_est_N1d-Clphi1d)*lls**4/(2*np.pi))
...
for i in range(N1,N2):
	namef = 'out_n0_1_1d_'+str(i)+'.txt'
	lls, Clphi, Clphi_est_N, Clphi_cross, yerr, Clphi_est_N1d, Clphi1d = np.loadtxt(namef,unpack=True)
	CEST = (Clphi_est_N1d-Clphi1d)*lls**4/(2*np.pi)#(Clphi_est_N-Clphi)*lls**4/(2*np.pi)
	CSTD += (CEST-CMEAN)**2
...
cl_Tphi = cl_Tphi[:len(cl['TE'])]/TCMB
...
for noiseFactor in [0.1]:
	...
	plt.semilogy(ellsx,ellsx**2.*(np.abs(cl['TT'])+numpy.interp(ellsx,ll,nlI))/(13.*10800./np.pi)**2./2./numpy.pi,label=str(noiseFactor)+'GI')
...
plt.semilogy(ellsx,ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI)*0.)/(13.*10800./3.14)**2./(2*np.pi)),label='TT')
# polarization
...
plt.savefig('lensingNoise_0_1.pdf')
