...
for noiseFactor in [1.]:
	...
	print cl['TT']-cl['TT_unlen']
#	plt.semilogy(ellsx,ellsx**2.*((np.abs(cl['TT']-cl['TT_unlen']-2*ellsx/np.sqrt(2)*np.abs(cl_Tphi)*(13.*10800./np.pi))+numpy.interp(ellsx,ll,nlI))/(13.*10800./np.pi)**2./2/np.pi),label=str(noiseFactor)+'GI') # i think no cross
	plt.semilogy(ellsx,ellsx**2.*((np.abs(cl['TT'])+numpy.interp(ellsx,ll,nlI))/(13.*10800./np.pi)**2./2/np.pi),label=str(noiseFactor)+'GI')
...
plt.semilogy(ellsx,cl_phiphi*ellsx**4/2./numpy.pi/TCMB**2,ls='dashed',color='k',label='Lensing power')
