...
savef = 'power_filt_subtr_n0_01_realCl.pdf'
savefn = 'power_noise_n0_01.pdf'
saverat = 'power_rat_filt_subtr_n0_01_realCl.pdf'
...
for i in (range(Nx)[:i_end+1]+range(Nx)[-i_end:]):
    for j in range(Ny)[:i_end+1]:
        ...
        if (ell[i,j] > l_last or ell[i,j] < l_beg):
            ...
            print "Nphiphi", np.mean(N_phiphi)
            print "lx,ly", lx[i],ly[j]
            print "clphi", Clphi_ar[i,j]
            W_ell1 = Clphi_ar[i,j]/(N_phiphi+Clphi_ar[i,j])
            W1.data = W_ell1
            W1 = W1.filterFromList([Larr,filt])
            W_ell2 = 1./N_phiphi
            W2.data = W_ell2
            W1.plot(show=False,saveFig = '../lala20000_W.png'); plt.close()
            W2.plot(show=False,saveFig = '../lala20000.png'); plt.close()
            quit()
