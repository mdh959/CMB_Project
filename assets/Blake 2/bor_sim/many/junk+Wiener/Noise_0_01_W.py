...
for i in range(N):
	namef = 'sims/Clphi_0_01_W_'+str(i)+'.txt'
	lls, Clphi, Clphi_est_W, Clphi_cross, yerr = np.loadtxt(namef,unpack=True)
	CEST = (Clphi_est_W-Clphi)*lls**4/(2*np.pi)
	CMEAN += CEST
CMEAN /= N
for i in range(N):
	namef = 'sims/Clphi_0_01_W_'+str(i)+'.txt'
...
plt.errorbar(lls,CMEAN,yerr=CSTD,label='Simulation Wiener')
plt.yscale('log')
...
plt.savefig('lensingNoise_0_01_30_W.pdf')
