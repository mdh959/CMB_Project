...
for noiseFactor in [1.]:
	...
	plt.semilogy(ellsx,ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI))/(12.37*10800./np.pi)**2.)/2./np.pi,label=str(noiseFactor)+'GI')
...
N_phiphi = NL_KK*4/Ls**4
N_phiphi[0] = N_phiphi[1] 
#f1 = open('QE_0001.txt', 'w+')
#for i in range(len(Ls)):
#	print >> f1, ('% 8.1f' % Ls[i]), ('% 18.8e' % N_phiphi[i])
#quit()


'''
z = np.polyfit(Ls,N_phiphi,4)
p = np.poly1d(z)
N_phiphi = p(Ls)
'''
noise = phi_new3.copy()

# VERY VERY IMPORTANT NAN OTHERWISE
Ls[0] = 0.00001
noise.fillWithGaussianRandomField(Ls,N_phiphi)#NL_KK*4/Ls**4)#,bufferFactor=1)

phi_new3.data[:,:] += noise.data[:,:]
print np.max(phi_new3.data)
print np.min(phi_new3.data)
phi_new3 = phi_new3.filterFromList([Larr,filt])
print phi_new3.data[:3,:3]

phi_new3.plot(show=False,saveFig = '../lala4.png',valueRange=[-1.e-8,1.e-8])
plt.close()

noise = phi_new3.copy()
noise.fillWithGaussianRandomField(Ls,N_phiphi)#,bufferFactor=1)
phi_new4.data[:,:] += noise.data[:,:]

phi_new4 = phi_new4.filterFromList([Larr,filt])
phi_new4.plot(show=False,saveFig = '../lala5.png',valueRange=[-1.e-8,1.e-8])
plt.close()
...
plt.errorbar(lls,CMEAN,yerr=CSTD,label='Simulation 1/N')
plt.yscale('log')
#plt.semilogy(lls,(Clphi_est-Clphi)*lls**4/(2*np.pi),label='Simulation 1/N')
#plt.semilogy(lls,(Clphi_est_W-Clphi)*lls**4/(2*np.pi),label='Simulation Wiener')
plt.xlim(1000, 20000)
plt.ylim(2.e-16,2.e-7)
plt.legend(loc='best')
plt.xlabel(r'$L$')
plt.ylabel(r'$l^2 N_l^{dd}/2 \pi$')
plt.savefig('lensingNoise_1_0_25.pdf')
