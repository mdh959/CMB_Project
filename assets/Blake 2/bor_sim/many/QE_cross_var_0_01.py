...
CSTD = np.zeros(45)
for i in range(N):
	namef = 'sims/Clphi_0_01_N_'+str(i)+'.txt'
	lls, Clphi, Clphi_est_N, Clphi_cross, yerr = np.loadtxt(namef,unpack=True)
	Clphi_cross[0] = 1.
	CEST = (Clphi_cross)*lls**4/(2*np.pi)#/4 for kappa
	Clphi *= lls**4/(2*np.pi)
	CMEAN += CEST
CMEAN /= N
for i in range(N):
...
#print CSTD
print Clphi/CMEAN
C_t_t = clphi*lphi**4/(2*np.pi) #/4 for kappa
...
# B.H.
cl['TT'] = cl['TT'][:len(cl['TT_unlen'])]
cl['EE'] = cl['EE'][:len(cl['EE_unlen'])]
cl['BB'] = cl['BB'][:len(cl['BB_unlen'])]
cl['TE'] = cl['TE'][:len(cl['TE_unlen'])]
...
for noiseFactor in [0.01]:
	...
	#plt.semilogy(ellsx,2*ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI))/(12.37*10800./3.14)**2.),label=str(noiseFactor)+'GI')
	...
	N_GI = ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI))/(12.37*10800./numpy.pi)**2.)/(2.*numpy.pi)
	N_QE  = NL_KK*4./2./numpy.pi

print Ls[0], Ls[-1]
print lphi[0], lphi[-1]
C_t_t_GI = np.interp(ellsx,lphi,C_t_t)
C_r_r_GI = (C_t_t_GI)+N_GI
C_t_t = np.interp(Ls,lphi,C_t_t)
C_r_r = (C_t_t)+N_QE


diff = np.diff(Ls)
diff = np.append(diff,diff[-1])
diff_GI = np.diff(ellsx)
diff_GI = np.append(diff_GI,diff_GI[-1])

D_r_t = np.sqrt((C_t_t**2+C_t_t*C_r_r)/((2.*Ls)*diff*fsky))
D_r_t_GI = np.sqrt((C_t_t_GI**2+C_t_t_GI*C_r_r_GI)/((2.*ellsx)*diff_GI*fsky))


plt.title("STD of Cross Power Spectrum")
plt.plot(Ls,D_r_t,label='Theory QE, n = '+str(noiseFactor))
plt.plot(ellsx,D_r_t_GI,label='Theory QE, n = '+str(noiseFactor))
plt.plot(lls,CSTD,label='Simulation 1/N, n = '+str(noiseFactor))
plt.yscale('log')
plt.xlim(1000, 20000)
plt.ylim(2.e-16,2.e-7)
plt.legend(loc='best')
plt.xlabel(r'$L$')
plt.ylabel(r'$l^2 \Delta_l^{dd}/2 \pi$')
plt.savefig('cross1.pdf')
plt.close()

# temperature used to be 13 tuk
#plt.semilogy(ellsx,ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI)*0.)/(12.37*10800./3.14)**2.),label='TT')
# polarization
...
#plt.semilogy(Ls,C_t_t,ls='dashed',color='r',label='Lensing power sim')
plt.errorbar(lls,CMEAN,yerr=CSTD,label='Simulation 1/N, n = '+str(noiseFactor))
plt.yscale('log')
plt.xlim(1000, 20000)
plt.ylim(2.e-16,2.e-7)
plt.legend(loc='best')
