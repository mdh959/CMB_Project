...
nametln = '/users/boryanah/Blake/bor_sim/many/sims/Tlens_S4_'+str(myrank)+'.fits'
namepn = '/users/boryanah/Blake/bor_sim/many/sims/phi_new_'+str(myrank)+'.fits'
savekappa = '/users/boryanah/Blake/bor_sim/many/sims/kappa_S4_'+str(myrank)+'.fits'
savekappaest = '/users/boryanah/Blake/bor_sim/many/sims/kappa_est_S4_'+str(myrank)+'.fits'
...
fname = 'Clphi_S4_N_'+str(myrank)+'.txt'
...
l=l[:lMax]
lt = l[:]
cl_TT=cl_TT[:lMax]*2*np.pi/(l*(l+1))
cl_EE=cl_EE[:lMax]*2*np.pi/(l*(l+1))
cl_TE=cl_TE[:lMax]*2*np.pi/(l*(l+1))
...
'''
ps2d = fftTools.powerFromLiteMap(Tlens)                                                                                                                                                  
l, l, ll, ClT, l, l = ps2d.binInAnnuli('BIN_200_LOG')
plt.plot(ll,ClT,label='sim T')
plt.plot(l_len,cl_TT_len,label='the T')
plt.plot(lt,cl_TT,label='the uT')
plt.legend()
plt.yscale("log")
plt.savefig('sp.png')
plt.close()
...
'''
plt.plot(ll,Clphi,label='sim')
plt.plot(lphi,clphi,label='the')
plt.yscale("log")
plt.legend()
plt.savefig('sp2.png')
plt.close()
...
for i in range(len(ll)):
    print >> f1, ('% 8.1f' % ll[i]), ('% 18.8e' % Clphi[i]), ('% 18.8e' % Clphi_est[i]), ('% 18.8e' % Clphi_cross[i]), ('% 18.8e' % yerr[i])
