...
cl_TT = numpy.interp(ellsx,ells,cl['TT'])
...
for noiseFactor in [0.01,0.1,1.]:
	...
	q = { 	'L_lens_min' : 2,
			'L_lens_max_temp' : 30000,
			'L_lens_max_pol' : 30000,
		}
	...
	plt.semilogy(ellsx,ellsx**2.*((cl_TT+numpy.interp(ellsx,ll,nlI))/(13.*10800./3.14)**2./(2.*numpy.pi))-0.5*cl_phiphi/(2*numpy.pi*TCMB**2),label=str(noiseFactor)+'GI')
	
plt.semilogy(ellsx,ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI)*0.)/(13.*10800./3.14)**2.)/(2*numpy.pi),'r--',label='TT')
plt.semilogy(ellsx,cl_phiphi/2./numpy.pi/TCMB**2.,'k--',label='Lensing power')
plt.xlim(1000, 20000)
