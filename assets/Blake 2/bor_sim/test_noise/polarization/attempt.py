...
Bulens = phi.copy()
Bulens.data[:,:] = 0.
FTulens = liteMap.fftFromLiteMap(Qlens)

ell = Fphi.modLMap
angLMap = Fphi.angLMap

# have looked at code and firsT is not used
FTulens, FQulens, FUulens = fftPol.fourierTEBtoFourierTQU(FTulens,FEulens,FBulens,modLMap,angLMap):

T_map = phi.copy()
Q_map = phi.copy()
U_map = phi.copy()
T_map.data[:,:] = FTulens.mapFromFFT()
Q_map.data[:,:] = FQulens.mapFromFFT()
U_map.data[:,:] = FUulens.mapFromFFT()

Tlens, Elens, Blens  =  lensMaps(phi_new,Tulens,Eulens,Bulens)
FTlens, FQlens, FUlens = fftPol.fourierTEBtoFourierTQU(FTlens,FElens,FBlens,modLMap,angLMap):
...
# ft of Q and U
FQlens = liteMap.fftFromLiteMap(Qlens)
FUlens = liteMap.fftFromLiteMap(Ulens)
...
for i in (range(Nx)[:i_end+1]+range(Nx)[-i_end:]):
    for j in range(Ny)[:i_end+1]:
        if (ell[i,j] > l_last or ell[i,j] < l_beg):
            ...
            # gradT dot ell
            gprod = (gradQux.data*lx[i]+ gradQuy.data*ly[j])
            ...
            phi_est.data = W_ell*Elens.data/(gprod)
...
Nl_pred_u = (ClE_u+ClnE)/((rms_Q**2+rms_U**2)*ll**2)
