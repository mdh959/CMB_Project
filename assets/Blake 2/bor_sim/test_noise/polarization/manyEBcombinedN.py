...
myrank = 'Pol_'+str(myrank)

noiseFactor = .9999

pwd = '/users/boryanah/Blake/bor_sim/test_noise/polarization/sims/'
...
for i in (range(Nx)[:i_end+1]+range(Nx)[-i_end:]):
    for j in range(Ny)[:i_end+1]:
        if (ell[i,j] > l_last or ell[i,j] < l_beg):
            ...
            gprod_B = (gradBux.data*lx[i]+ gradBuy.data*ly[j])
...
Nl_pred_u = 1./((rms_E*ll)**2/(ClE_u+ClnE)+(rms_B*ll)**2/(ClnB))
