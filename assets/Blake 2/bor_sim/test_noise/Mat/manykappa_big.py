...
if noiseFactor == .9999: # CMB-S4 conservative
    fname = pwd+'Clphi_S4_cons_k_'+str(myrank)+'_Mat_big.txt'
    S4 = '_S4'
    thetaFWHMarcmin = 1.
...
if noiseFactor == .1:
    fname = pwd+'Clphi_0_1_k_'+str(myrank)+'_Mat_big.txt'
    S4 = ''
    thetaFWHMarcmin = .3
...
if noiseFactor == .5:
    fname = pwd+'Clphi_0_5_k_'+str(myrank)+'_Mat_big.txt'
...
nois = enmap.read_fits(fname_noise+str(myrank)+S4+'_'+str(area)+'.fits')
lensed = enmap.read_fits(fname_lensed+str(myrank)+S4+'_'+str(area)+'.fits')
grad_T_unlensed = enmap.read_fits(fname_grad_T_unlensed+str(myrank)+S4+'_'+str(area)+'.fits')
# these are the not downsampled ones
unlensed = enmap.read_fits(fname_unlensed+str(myrank)+S4+'_'+str(area)+'.fits')
...
#l, l, ll, ClT_u, l, l = p2d.binInAnnuli('BIN_1000')

# noise power spectrum
p2d = fftTools.powerFromLiteMap(noise)
l, l, ll, Cln, l, l = p2d.binInAnnuli('BIN_1000')


# lensed temperature power spectrum
p2d = fftTools.powerFromLiteMap(Tlens)
l, l, ll, ClT_l, l, l = p2d.binInAnnuli('BIN_1000')


# lensing potential power spectrum
ps2d = fftTools.powerFromLiteMap(phi)
l, l, ll, Clphi, l, l = ps2d.binInAnnuli('BIN_1000')
...
l, l, ll, Clkappa, l, l = ps2d.binInAnnuli('BIN_1000')
...
l, l, ll, Clphi_est, l, l = ps2d_est.binInAnnuli('BIN_1000')

ps2d_cross = fftTools.powerFromFFT(Fphi,Fphi_est_unnorm)
l, l, ll, Clphi_cross, l, l = ps2d_cross.binInAnnuli('BIN_1000')
