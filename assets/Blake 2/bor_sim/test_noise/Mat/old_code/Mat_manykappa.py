...
for i in range(nproc):
    print "LLALALA, ",i
    # downsampled 0.1 arcmin pix and 1x1 deg^2
    noise = enmap.read_fits(fname_noise+str(myrank)+'.fits')
    lensed = enmap.read_fits(fname_lensed+str(myrank)+'.fits')
    grad_T_unlensed = enmap.read_fits(fname_grad_T_unlensed+str(myrank)+'.fits')
    # these are the not downsampled ones
    unlensed = enmap.read_fits(fname_unlensed+str(myrank)+'.fits')
    phi = enmap.read_fits(fname_phi+str(myrank)+'.fits')

# as of now thethaFWHM is 0.3 and noise is 0.1 uK-arcmin
noiseFactor = .1
pwd = '/users/boryanah/Blake/bor_sim/test_noise/Mat/sims/Mat_'
...
# TUK
#template = liteMap.makeEmptyCEATemplate(1.706667, 1.706667,meanRa = 180., meanDec = 0.,\
template = liteMap.makeEmptyCEATemplate(.5*1.7066666667, .5*1.7066666667,meanRa = 180., meanDec = 0.,\
                      pixScaleXarcmin = .1, pixScaleYarcmin = .1)

#template = liteMap.makeEmptyCEATemplate(1, 1,meanRa = 180., meanDec = 0.,\
#                      pixScaleXarcmin = .1, pixScaleYarcmin = .1)

Tlens = template.copy()
phi_new = template.copy()

Tlens.data = lensed
# might wanna revisit with to flipper
phi_new.data = phi
phi = phi_new.copy()
...
print grad_T_unlensed.shape
gradTux = template.copy()
gradTuy = template.copy()

gradTux = enmap.to_flipper(grad_T_unlensed[0,:,:])
gradTuy = enmap.to_flipper(grad_T_unlensed[1,:,:])

#gradTl = gradf(FTlens_filt)
#grad_T_lensed = enmap.grad(lensed)
...
'''
# CHANGE THIS
Tulens = enmap.to_flipper(unlensed)
