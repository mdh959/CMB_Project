...
if noiseFactor == .1:
    ...
    name_kappa_QE = 'kappa_rec_0_1_big.fits'
    kappa = liteMap.liteMapFromFits(name_kappa)
    kappa_est = liteMap.liteMapFromFits(name_kappa_est)
    kappa_QE = enmap.read_fits(name_kappa_QE)
    kappa_qe = kappa.copy()
    ...
    phi_est.plot(show=False,saveFig = pwd+'phi_est_0_1.png',valueRange=[np.min(phi.data),np.max(phi.data)])
    plt.figure(1)
    #phi_est.plot(show=False,saveFig = pwd+'phi_est_0_1.pdf',valueRange=[np.min(phi_est.data),np.max(phi_est.data)])
    plt.close()
    
    plt.figure(2)
    phi_qe.plot(show=False,saveFig = pwd+'phi_qe_0_1.png',valueRange=[np.min(phi.data),np.max(phi.data)])
    ...
    plt.figure(3)
    phi.plot(show=False,saveFig = pwd+'phi_real_0_1.png',valueRange=[np.min(phi.data),np.max(phi.data)])
