...
if noiseFactor == .1:
    ...
    fname = 'Clphi_lenT_0_1_k_'+str(myrank)+'_Mat'+str(dl)+'_big.txt'
    S4 = ''
    SO = ''
    thetaFWHMarcmin = .3
...
l, l, ll, Cln, l, l = p2d.binInAnnuli('BIN_'+str(dl)+SO)
...
plt.plot(ll,ll**2*ClT_l,label='mine')
...
Tlens_noN, Qlens, Ulens  =  lensMaps(phi,Tulens,Tulens,Tulens, TaylOrder = 120)
Tlens = Tlens_noN.copy()
Tlens.data[:,:]  +=  noise.data[:,:]

Tlens.writeFits('Tlens_Thibaut.fits',overWrite=True)
'''
Tlens = liteMap.liteMapFromFits('Tlens_Thibaut.fits')

p2d = fftTools.powerFromLiteMap(Tlens)
l, l, ll, ClT_l, l, l = p2d.binInAnnuli('BIN_'+str(dl)+SO)
plt.plot(ll,ll**2*ClT_l,label='lensingTools')
