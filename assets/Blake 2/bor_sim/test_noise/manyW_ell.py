...
filt[Larr>2000.] = 0.
filt[0] = 0.
# here we get more reali-STICK i.e. use T lens
...
# initiate estimator in real space
phi_est = phi.copy()
phi_est.data[:,:] = 0
...
for i in (range(Nx)[:i_end+1]+range(Nx)[-i_end:]):
    for j in range(Ny)[:i_end+1]:
        if (ell[i,j] > l_last or ell[i,j] < l_beg):
            ...
            # the i factor in the equations
            norm_c *= 1j
            norm *= 1j
            
            # estimator in real space
            phi_est.data = W_ell*Tlens.data/(gprod)
            # fourier transform and keep l = l'
...
# old def of auto, will be changed
...
# not really important just from before
diff = np.diff(ll)
...
# prediction for the noise power spectrum
