...
#Fphi.plot(show=True,log=True,zoomUptoL=20000); plt.show(); plt.savefig('Fphi.png');plt.close()
#quit()
...
for i in (range(Nx)[:i_end+1]+range(Nx)[-i_end:]):
    for j in range(Ny)[:i_end+1]:
        if (ell[i,j] > l_last or ell[i,j] < l_beg):
            ...
            lala_1.kMap[j,i] = norm_c
            lala_sq2.kMap[j,i] = norm
            lala_1.kMap[-j,-i] = np.conj(norm_c)
            lala_sq2.kMap[-j,-i] = np.conj(norm)
            ...
            #Fphi_est_unnorm.kMap[j,i] = Fphi_est_ell.kMap[j,i]/1j
            Fphi_est.kMap[j,i] = Fphi_est_ell.kMap[j,i]/norm
            #Fphi_est.kMap[j,i] = Fphi_est_ell.kMap[j,i]/1j
...
#Fphi_est.plot(show=True,log=True,zoomUptoL=20000);plt.show();plt.savefig('Fphi.png');plt.close()

#lala_sq2.plot(show=True,log=True,zoomUptoL=20000);plt.show();plt.savefig('sqavW2.png');plt.close()
lala_sq2.plot(show=False,log=True,zoomUptoL=20000);plt.savefig('sqavW2.png');plt.close()
#lala_1.plot(show=True,log=True,zoomUptoL=20000); plt.show();plt.savefig('avW.png');plt.close()
lala_1.plot(show=False,log=True,zoomUptoL=20000);plt.savefig('avW.png');plt.close()
#lala_1.kMap[:,:] = lala_1.kMap[:,:] - lala_sq2.kMap[:,:]
#lala_1.plot(show=False,log=True,zoomUptoL=20000);plt.savefig('diffavW.png');plt.close()
...
#ps2d_est = fftTools.powerFromFFT(lala_sq2)
ps2d_est.powerMap[:,:] = lala_sq2.kMap[:,:]
l, l, ll, norm_auto, l, l = ps2d_est.binInAnnuli('BIN_200_LOG')

#ps2d_est = fftTools.powerFromFFT(lala_1)
ps2d_est.powerMap[:,:] = lala_1.kMap[:,:]
l, l, ll, norm_cross, l, l = ps2d_est.binInAnnuli('BIN_200_LOG')

norm = norm_auto/norm_cross
print norm
...
filt[Larr<5000.] = 0.
filt[Larr>20000.] = 0.
Fphi_est_unnorm = phi.filterFromList([Larr,filt])
Fphi_est_unnorm.kMap[:,:] /= norm
'''

ps2d_est = fftTools.powerFromFFT(Fphi_est_unnorm)
l, l, ll, Clphi_est, l, l = ps2d_est.binInAnnuli('BIN_200_LOG')
...
'''
#tuka
Clphi_est /= norm_auto**2
Clphi_est /= (norm_cross)
'''
