...
if noiseFactor == .1:
    ...
    QEname = QEdir+'QE_0_1.txt'
    print noiseFactor
    thetaFWHMarcmin = 0.3

if noiseFactor == .0001:
    nametu = '/users/boryanah/Blake/bor_sim/test_noise/T_map_0_0001.fits'
    nametl = '/users/boryanah/Blake/bor_sim/test_noise/Tlens_0_0001.fits'
    nametln = '/users/boryanah/Blake/bor_sim/test_noise/Tlens_noise_0_0001.fits'
    namen = '/users/boryanah/Blake/bor_sim/test_noise/noise_0_0001.fits'
    namepn = '/users/boryanah/Blake/bor_sim/test_noise/phi_new_0_0001.fits'
...
filt[0] = 0.
# here we get more reali-STICK
# VAJNO DA SE filtrira
Tlens_filt = Tulens.filterFromList([Larr,filt])
# not different from
#Tlens_filt = Tlens_noN.filterFromList([Larr,filt])
# gradient of unlensed temperature
...
N_QE = np.interp(ll,Ls,N_QE)
clphi = np.interp(ll,lphi,clphi)
lala = (clphi-N_QE)/clphi

plt.semilogy(ll, N_QE, label='N_QE')
plt.semilogy(ll, clphi, label='Clphi')
plt.xlim([5000,20000])
plt.ylim([1.e-27,1.e-23])
plt.legend()
plt.savefig("lala.png"); plt.close()
...
'''
plt.semilogy(ll, ratio, label='ClT_l/ClT_u')
plt.semilogy(ll, rat, label='N_QE/N_GI')
plt.xlim([5000,20000])
#plt.ylim([1.e-27,1.e-23])
plt.legend()
plt.savefig("lala1.png"); plt.close()

print ratio
print rat
print (ratio - rat)/ratio
quit()
#TUKA
'''
...
# TRY TO GET RID OF RINGING 4000
Larr = np.arange(40000.)
filt = np.ones(len(Larr))
filt[Larr<6000.] = 0.
...
for i in (range(Nx)[:i_end+1]+range(Nx)[-i_end:]):
    for j in range(Ny)[:i_end+1]:
        if (ell[i,j] > l_last or ell[i,j] < l_beg):
            ...
            if ell[i,j] < 40500 and ell[i,j] > 38000:
                fsky = 1
                #W_ell[:,:] = 1.
                #x_inv[:,:] = 1.
                f_W1 = np.mean(np.abs(W_ell))
                f_W2 = np.mean(W_ell**2)
                f_W4 = np.mean(W_ell**4)
                eff_area = np.sqrt(f_W4/(f_W2**2*fsky))
                eff_area_cr = np.sqrt(f_W2/(f_W1**2*fsky))
                f_W2_inv = np.mean(x_inv**2)
                f_W4_inv = np.mean(x_inv**4)
                eff_area_inv = np.sqrt(f_W4_inv/(f_W2_inv**2*fsky))
                eff = np.sqrt(1./fsky)
                print "1./eff_area_cr, 1./eff_area_inv, 1./eff "
                print 1./eff_area_cr, 1./eff_area_inv, 1./eff 
                quit()
            ...
            '''
            '''
            #norm = np.sqrt(np.mean(W_ell.data**2))
            norm = np.sqrt(np.mean(W_ell**2))
            #norm_c = np.mean(W_ell.data)
            ...
            # MNOGO VAJNA PROMQNA NORM_C A TR DA E NORM
...
#ps2d_cross = fftTools.powerFromFFT(Fphi,Fphi_est)
...
# TUK
#ps2d_cross = fftTools.powerFromLiteMap(phi_est)
#l, l, ll, Clphi_est, l, l = ps2d_cross.binInAnnuli('BIN_200_LOG')

'''
# noise choose one of two!!
...
diff = Fphi.copy()
diff.kMap[:,:] = 0
diff.kMap[:,:] = Fphi_est.kMap[:,:] - Fphi.kMap[:,:]

ps2d_cross = fftTools.powerFromFFT(diff)
l, l, ll, Nlphi, l, l = ps2d_cross.binInAnnuli('BIN_200_LOG')
...
for i in range(len(ll)):
    print >> f1, ('% 8.1f' % ll[i]), ('% 18.8e' % Clphi[i]), ('% 18.8e' % Clphi_est[i]), ('% 18.8e' % Clphi_cross[i]), ('% 18.8e' % rat[i]),('% 18.8e' % Nl_pred_u[i]), ('% 18.8e' % ratio[i]), ('% 18.8e' % Nlphi[i])
...
phi_QE = phi.copy()
phi_QE.fillWithGaussianRandomField(ll,N_QE)
phi_QE.data[:,:] += phi.data
phi_QE = phi_QE.filterFromList([Larr,filt])

if noiseFactor == .0001:
    phi_est.plot(show=False,saveFig = 'phi_est_0_0001.png',valueRange=[np.min(phi.data),np.max(phi.data)]);plt.close()
    phi.plot(show=False,saveFig = 'phi_real_0_0001.png');plt.close()
