...
for i in range(45):
    print l[i],make[i], ti[i], make2[i]

plt.figure()
plt.plot(l,make,'k--',label=r"$C_l^{\hat \psi \hat \psi}-2 C_l^{\hat \psi \psi}+C_l^{\psi \psi}$")
plt.semilogy(l,make2,label=r'$C_l^{\hat \psi \hat \psi}-C_l^{\psi \psi}$');
plt.semilogy(l,ti,label=r'$C_l[\hat \psi - \psi]$')
plt.legend(loc='best')#;plt.title(Ntitle)
plt.xlim([1,20000])
plt.xlabel(r"$l$");plt.ylabel(r"$N_l^{\psi \psi}$")
plt.savefig('lala2.png')#;plt.show()#;plt.close()
