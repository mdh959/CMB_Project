...
for noiseFactor in [0.01,0.1,1.]:
	...
	# B.H. very important to notice difference for MATHEW
	...
	# B.H. this is for printing QE
	#v = np.vstack((Ls,NL_KK))
	#v = np.transpose(v)
	#np.savetxt('QE_0_5_Mat.txt',v)
	# tuk
	...
	fac /= den
	plt.semilogy(ellsx,ellsx**2.*fac*4./(1.9*10800./3.14)**2.,label=str(noiseFactor)+'GI')
	
# temperature
#plt.semilogy(ellsx,ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI)*0.)/(13.*10800./3.14)**2.),label='TT')
# polarization
plt.semilogy(ellsx,ellsx**2.*((2.*numpy.abs(cl['EE_unlen'])+2.*numpy.interp(ellsx,ll,nlI)*0.)/(1.9*10800./3.14)**2.),label='EE')
...
#plt.savefig('lensingNoiseHighLpowerShare.png')
plt.savefig('lensingNoiseHighLpowerPol.png')
