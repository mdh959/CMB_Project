...
def noise_kernel_lim(theta, l1, L, field, cl_unlen, cl_tot, lmin, lmax):
	...
	if field == 'TT': 
		...
		#cl2_unlen, cl2_tot = get_cl2(cl_unlen['TT'], cl_tot['TT'], l2, lmin, lmax)
		# B.H. not sure
                #cL_unlen = cl_unlen['TT'][L]
		cL_tot = cl_tot['TT'][L]
		kernel = 2.*( cl1_unlen * Ldotl1 )**2 / (2 * cl1_tot * cL_tot)
	# only suported for TT for now
	'''
	elif field == 'TE': 
		cl2TT = get_cl2(cl_unlen['TT'], cl_tot['TT'], l2, lmin, lmax)[1]
		cl2_unlen, cl2_tot = get_cl2(cl_unlen['TE'], cl_tot['TE'], l2, lmin, lmax)
...
def get_lensing_noise_lim(l_array, cl_fid, nl, fields, q):
...
# B.H. might be a problem
	...
	if 'TT' in fields : 
		lensingSpec += ['TT']
	if 'TE' in fields:
		print "Currently supported only for TT..."
		quit()
#lensingSpec += ['TE','TB']
	if 'EE' in fields: # B.H. if only EE rm EB and fields BB
		print "Currently supported only for TT..."
		quit()#lensingSpec += ['EE', 'EB']
	if 'BB' in fields:
		print "Currently supported only for TT..."
		quit()
	#lensingSpec = ['TT','TE','EE','EB','TB']
	...
	for field in lensingSpec:
		...
		for L in Ls:
			...
			if field == 'TT':
				...
				Theta, Ells = np.meshgrid(thetas, l_array.astype(int)[L_lens_min - lmin:L_lens_max_pol - lmin])
			kernel_grid = noise_kernel_lim(Theta, Ells, L, field, cl_unlen, cl_tot, lmin, lmax)
			integral = deltaTheta * np.sum(np.sum(kernel_grid[np.where(kernel_grid > 0)], axis = 0), axis = 0)
			y.append(L*L*(integral)**(-1))
		NL[field] = np.array(y)
...
file = '/users/boryanah/Blake/Aug6_highAcc_CDM'
...
for noiseFactor in [0.01,0.1,1.]:
	...
	Ls, NL_KKlim = get_lensing_noise_lim(ells, cl, nl, fields, q)

	
	plt.semilogy(Ls, NL_KK*4./2./numpy.pi,label=str(noiseFactor)+'QE', color = str((noiseFactor/2.)**0.25))
	# B.H. QE limit
	plt.semilogy(Ls, NL_KKlim*4./2./numpy.pi,label=str(noiseFactor)+'QElim', ls='dashed', color = str((noiseFactor/2.)**0.25))
	...
	# B.H. 
	# for temperature
	plt.semilogy(ellsx,ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI))/(13.*10800./3.14)**2.),label=str(noiseFactor)+'GI')
...
# temperature
plt.semilogy(ellsx,ellsx**2.*((cl['TT_unlen']+numpy.interp(ellsx,ll,nlI)*0.)/(13.*10800./3.14)**2.),label='TT')
# polarization
#plt.semilogy(ellsx,ellsx**2.*((2.*numpy.abs(cl['EE_unlen'])+2.*numpy.interp(ellsx,ll,nlI)*0.)/(1.95*10800./3.14)**2.),label='EE')
plt.semilogy(ellsx,cl_phiphi/2./numpy.pi/TCMB**2.,ls='dotted',color='k',label='Lensing power')
plt.xlim(1000, 10000)
plt.ylim(2.e-16,2.e-7)
plt.legend(loc='best')
plt.xlabel(r'$L$')
plt.ylabel(r'$l^2 N_l^{dd}/2 \pi$')
plt.savefig('lensingNoiseQElimit.pdf')
