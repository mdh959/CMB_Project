...
def simulate_tebp_correlated(cl_tebp_arr,nside,lmax) :
	...
	print np.array(alms).shape
	aphi=alms[-1]
	# for polarization and stuff
	acmb=alms[0:-1]	
	# for temperature only
	#acmb=alms[0]	
#	np.array(acmb).flatten()
	#print np.array(aphi).shape
	#print np.array(acmb).shape
#Set to zero above map resolution to avoid aliasing
	...
	for ac in acmb :
		...
		#my shit b.h.
		#cmb=np.array(healpy.alm2map(np.array(acmb),nside,pol=True))
...
for sim in range(myid, args.nsim, nproc):
	...
	if "u" in args.output:
		...
		s=healpy.write_map("%s/ucmb%03d_%d.fits" % (args.odir, sim, args.order),
				 cmb)
		#liteMap.liteMapFromFits("%s/ucmb%03d_%d.fits" % (args.odir, sim, args.order))
		#s.writeFits('la.fits')
		#cmb.writeto('template.fits')
